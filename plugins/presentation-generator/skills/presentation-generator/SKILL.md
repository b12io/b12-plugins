...
description: Build a complete slide deck — pitch decks, sales decks, investor updates, quarterly business reviews, board updates, training and onboarding decks, project kickoffs, research readouts, all-hands, conference talks — delivered as a .pptx file, or a self-contained HTML deck when PowerPoint files cannot be written, with an argument running through it, one idea per slide, speaker notes, and marked placeholders in place of invented numbers. Use when someone wants slides, a deck, a presentation, or a PowerPoint, Keynote, or Google Slides file made, restructured, tightened, or cut to a time limit, including asks like "turn these notes into a deck" or "I'm presenting to the board on Thursday". Do NOT use for prose that is not slides — an email, a post, web copy, a written report or one-pager — that is a separate writing skill. Do NOT use for designing a web page or mockup, for generating a photo, hero image, or OG image, for drawing a logo, mark, or favicon, for writing source code, or for building a blog or website; those are separate skills too. "Design my slides" is still this skill; "make an image for slide 3" is the image skill.
---
